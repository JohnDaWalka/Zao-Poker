import { BLIND_LEVELS } from './types.js';
function isStreetOver(activePlayers, tableBet) {
...
function getNextActiveIndex(currentIndex, activePlayers) {
...
export function encodeActionEntry(actor, action, amount) {
...
export function parseActionHistory(raw) {
    return String(raw || "")
        .split("|")
        .map((entry) => entry.trim())
        .filter(Boolean);
}
export function getCurrentBlinds(startTimeStr) {
    if (!startTimeStr)
        return { levelIndex: 0, blinds: BLIND_LEVELS[0], nextLevelInSecs: 360 };
...
export function getBasicAiDecision(toCall, potSize, stackSize, phase) {
    if (toCall === 0) {
...
export async function dealNewHand(db, tableId, fids) {
    ...
    const seatedPlayers = seatedRows.filter((p) => Number(p.stack_size || 0) > 0);
    ...
    const getRelative = (offset) => seatedPlayers[(newDealerSeatIndex + offset) % seatedPlayers.length];
    const sbPlayer = getRelative(1);
    const bbPlayer = getRelative(2);
    const sbFid = Number(sbPlayer.fid);
    const bbFid = Number(bbPlayer.fid);
    const firstToActPlayer = seatedPlayers.length === 2 ? sbPlayer : getRelative(3);
    ...
    for (const player of seatedPlayers) {
        ...
        let blindAmount = 0;
        if (fid === sbFid)
            blindAmount = blinds.sb;
        if (fid === bbFid)
            blindAmount = blinds.bb;
...
export async function advanceGame(db, tableId, currentState) {
    ...
    if (nextPhase !== "showdown") {
        ...
        if (activePlayers.length > 0) {
            const startIdx = activePlayers.findIndex((p) => Number(p.seat_index) > dealerSeatIndex);
