import { rankShowdownWinners, rankHighLowShowdown, } from './poker-evaluator.js';
import { getGameConfig } from './game-rules.js';
async function getHandParticipants(db, tableId) {
...
export function buildSidePots(activePlayers) {
    const eligible = activePlayers.filter((p) => p.invested > 0);
    if (eligible.length === 0)
        return [];
    const sortedInvestments = Array.from(new Set(eligible.map((p) => p.invested))).sort((a, b) => a - b);
    const pots = [];
    let previousTier = 0;
...
export async function resolveFoldWin(db, tableId, winnerFid, potSize, board, phaseReached) {
...
export async function resolveShowdown(db, tableId, potSize, board, variant = "NLHE") {
    ...
    const activePlayers = activeRows.map((row) => ({
        fid: Number(row.fid),
        username: String(row.username || `User#${row.fid}`),
        holeCards: String(row.hand || ""),
        invested: Number(row.total_invested || 0),
    ...
    if (config.showdownType === "high-low") {
        const showdownPlayers = activePlayers.map((p) => ({
            fid: p.fid,
            username: p.username,
            holeCards: p.holeCards.split(",").filter(Boolean),
            invested: p.invested,
        }));
        for (const pot of pots) {
            const eligibleShowdown = showdownPlayers.filter((p) => pot.eligibleFids.includes(p.fid));
            if (eligibleShowdown.length === 0)
                continue;
            const { highWinners, lowWinners } = rankHighLowShowdown(eligibleShowdown, boardCards, variant);
        ...
        for (const pot of pots) {
            ...
            if (eligiblePlayers.length === 0)
                continue;
            const winnerIndices = rankShowdownWinners(eligiblePlayers.map((p) => ({ holeCards: p.holeCards.split(",").filter(Boolean) })), boardCards);
...
async function recordHandOutcome(db, tableId, board, potSize, phaseReached, resolution, outcomes) {
